...
export function calculateCartTotalPrice(cart: CartFull) {
  return cart
    ? cart.items.reduce((acc, { product: { price = 0 }, count }) => {
...
export async function updateByUserId(userId: string, cart: Partial<CartEntity | null>) {
  ...
  if (!updatedCart) {
    throw new ValidationException({ message: 'Cart was not updated' });
  }
