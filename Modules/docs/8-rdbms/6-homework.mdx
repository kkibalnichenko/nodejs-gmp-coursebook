...
<TableForMentors data={[
  {
    items: [
      {
        text: "Data is stored in PostgreSQL database. Docker image is used for local development",
        points: 1,
      },
      {
        text: "All existing endpoints are switched to use PostgreSQL database",
        points: 5,
      },
      {
        text: "Migrations are used to create and delete tables",
        points: 2,
      },
      {
        text: "Seeds are used to populate database with test data",
        points: 2,
      }
    ]
  },
